...
## 4.5. Вход через Google и Telegram (опционально)

Email/пароль работает всегда. Если хочешь, чтобы знакомые заходили в один
клик — настрой одно или оба:

**Google:**
1. console.cloud.google.com → создай проект (если ещё нет) → APIs & Services
   → Credentials → **Create Credentials → OAuth client ID** → тип
   **Web application**.
2. Authorized JavaScript origins — впиши свой домен (Railway-домен или
   `http://localhost:8000` для локального теста).
3. Скопируй **Client ID** → `GOOGLE_OAUTH_CLIENT_ID` в `.env`.

**Telegram:**
1. В Telegram напиши @BotFather → `/newbot` → получишь `TELEGRAM_BOT_TOKEN`.
2. Обязательно выполни `/setdomain` у @BotFather и укажи домен сайта —
   без этого виджет входа не отобразится и не заработает.
3. Юзернейм бота (без `@`) → `TELEGRAM_BOT_USERNAME` в `.env`.

Первый раз при входе через Google/Telegram нужен тот же `INVITE_CODE`, что
и при обычной регистрации — так посторонние не смогут завести аккаунт
через кнопку OAuth. При повторном входе инвайт-код уже не спрашивается.

## 4.6. Админка

Первый зарегистрированный (любым способом — email, Google или Telegram)
автоматически становится админом и получает доступ без подписки. У админа
на дашборде появляется блок **«Админка»**:

- список всех пользователей (email, способ входа, статус подписки),
- кнопки **«Выдать доступ»** / **«Забрать доступ»** — включают или
  выключают доступ к генерации вручную, без прохождения через Stripe.
  Удобно, если хочешь пустить кого-то из знакомых бесплатно.
