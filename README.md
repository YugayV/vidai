...
6. Рядом с кнопкой «Скачать» — **«Скопировать ссылку»**: публичная ссылка
   вида `/jobs/{id}/public/{share_token}`, по ней можно скачать ролик без
   входа на сайт (удобно кинуть прямо в Telegram).
7. Блок **«Аналитика»** на дашборде — сколько роликов, сколько готово/с
   ошибкой/в процессе, средняя длительность генерации, график за 14 дней.
   У админа в «Админке» — та же аналитика, но по всем пользователям сразу
   плюс топ по количеству роликов.
8. При создании ролика можно выбрать формат: **9:16** (Reels/Shorts, по
   умолчанию), 1:1 или 16:9 — прокидывается в генерацию кадров и видео.
9. Для готовых роликов на дашборде теперь встроенный видео-плеер (не
   только ссылка на скачивание) и пошаговый индикатор статуса вместо
   пульсирующей полоски: Сценарий → Кадры → Видео → Озвучка → Сборка →
   Готово.
10. **Ассистент по темам (Claude)** — чат прямо на дашборде, помогает
    придумать тему/хук/сценарий для ролика. У ответов ассистента есть
    кнопка «Вставить в тему» — подставляет предложенную тему в форму
    создания ролика одним кликом.
11. **Нарезка клипов** — у готового ролика кнопка «Нарезать клип»:
    указываешь диапазон в секундах, получаешь отдельный .mp4-файл этого
    отрезка (например, чтобы сделать короткий тизер из части ролика).
